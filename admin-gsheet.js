...
// Chuyển link GSheet bất kỳ → embed URL (?rm=minimal)
function gSheetToEmbed(url) {
  if (!url) return '';
  // Lấy fileId từ mọi dạng link GSheet
  const m = url.match(/\/spreadsheets\/d\/([\w-]+)/);
  if (m) return `https://docs.google.com/spreadsheets/d/${m[1]}/edit?rm=minimal`;
  return url;
}
...
async function _gsThemUrl() {
  ...
  // Convert sang dạng chuẩn trước khi lưu
  const urlEmbed = gSheetToEmbed(url);
  try {
    await sb.from(CONFIG.BANG.DANH_SACH_BANG).update({ gsheet_url: urlEmbed }).eq('bang', bang);
    const b = ADMIN_CORE.dsBang.find(x => x.bang === bang);
    if (b) b.gsheet_url = urlEmbed;
...
async function _gsGanBang(bangCu, bangMoi, url) {
  ...
  try {
    ...
    if (bangMoi) {
      // Gắn sang bảng mới
      await sb.from(CONFIG.BANG.DANH_SACH_BANG).update({ gsheet_url: urlEmbed }).eq('bang', bangMoi);
      const bMoi = ADMIN_CORE.dsBang.find(x => x.bang === bangMoi);
      if (bMoi) bMoi.gsheet_url = urlEmbed;
